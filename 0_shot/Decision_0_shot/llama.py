...
tokenizer = AutoTokenizer.from_pretrained(
    "meta-llama/Meta-Llama-3-8B-Instruct", token=HUGGINGFACE_TOKEN, cache_dir=SCRATCH_DIR
)
...
def run(start=0, num_left=10000):
    ...
    for i, row in tqdm(data[start : start + num_left].iterrows(), total=total):
        ...
        try:
            ...
            row['Prediction'] = tokenizer.decode(response, skip_special_tokens=True)
...
if __name__ == "__main__":
    parser = ArgumentParser(prog="Llama")
